...
var DESCRIPTION = `Start an app and test it with a GET request.`;
var USAGE = `Usage:
  ${COMMAND.bold}
Description:
  ${DESCRIPTION}
...
exports.handler = function (opts) {
  ...
  if (!opts.start) {
    utils.testRequest(opts.url || `http://localhost:${opts.port || buildPack.config.test.app.port || 8080}`, opts).then(function () {
      return utils.logger.log(CLI_CMD, 'Test complete.'.green);
    }, function (err) {
      return utils.logger.error(CLI_CMD, 'Test failed.', err);
    });
    return;
  }

  utils.getPort(opts).then(function (port) {
    var options = {
      cwd: opts.localPath,
    ...
    // Start the app
    var child = childProcess.spawn(opts.cmd, opts.args, options).on('exit', function (code, signal) {
    ...
    setTimeout(function () {
      ...
      utils.testRequest(opts.url || `http://localhost:${options.env.PORT}`, opts).then(function () {
        return cleanup();
      }, function (err) {
        requestErr = err;
        cleanup();
