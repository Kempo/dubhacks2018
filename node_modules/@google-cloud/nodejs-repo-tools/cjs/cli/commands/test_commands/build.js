...
var tpl = path.join(__dirname, '../../../../templates/cloudbuild.yaml.tpl');

var CLI_CMD = 'build';
var COMMAND = `tools test ${CLI_CMD} ${'[options]'.yellow}`;
var DESCRIPTION = `Launch a Cloud Container build.`;
var USAGE = `Usage:
  ${COMMAND.bold}
Description:
  ${DESCRIPTION}
...
exports.handler = function (opts) {
  ...
  opts.requiredEnvVars = opts.requiredEnvVars.map(function (envVar) {
  ...
  try {
    ...
    var template = handlebars.compile(fs.readFileSync(tpl, 'utf8'))(opts);
    if (!opts.dryRun) {
    ...
    // Start the build
    var buildCmd = `gcloud container builds submit . --config 'repo-tools-cloudbuild.yaml' --project '${opts.builderProject}'`;
    if (opts.async) {
      buildCmd += ' --async';
    } else {
    ...
    if (!opts.dryRun) {
      ...
      try {
        ...
        // Remove temp files
        cleanup();
        var _timeTakenStr = utils.getTimeTaken(start);
        utils.logger.error(CLI_CMD, `Oh no! Build failed after ${_timeTakenStr}.`);
...
function getHeadCommitSha(cwd) {
  ...
  var stdout = childProcess.execSync('git log -n 1 --pretty=format:"%H"', {
    cwd,
    stdout: 'ignore',
    timeout: 20 * 60 * 1000
  });
