...
var CLI_CMD = 'install';
var INSTALL_CMD = buildPack.config.test.install.cmd;
var INSTALL_ARGS = buildPack.config.test.install.args;
var INSTALL_CMD_STR = `${INSTALL_CMD} ${INSTALL_ARGS.join(' ')}`.trim();
var COMMAND = `tools test ${CLI_CMD} ${'[options] [--] [args...]'.yellow}`;
var DESCRIPTION = `Run ${INSTALL_CMD_STR.bold} in ${buildPack._cwd.yellow}.`;
var USAGE = `Usage:
  ${COMMAND.bold}
Description:
  ${DESCRIPTION}
...
exports.builder = function (yargs) {
  ...
  }).example('Run the install command in the specified directory:').example(`- ${'tools test install -l=~/projects/some/dir'.cyan}`).example(`Runs ${'npm install --no-optional'.bold} instead of the default command:`).example(`- ${'tools test install --cmd=npm -- install --no-optional'.cyan}`);
};
